...
function resetShopForm() {
  ...
  if (form) {
    ...
    form.setAttribute('hx-target', '#shops-content');
    form.setAttribute('hx-swap', 'outerHTML');
    // Note: form reset is now handled by global htmx:afterRequest listener
...
document.body.addEventListener('htmx:afterRequest', function(event) {
  ...
  if (event.detail.target && event.detail.target.id === 'questions-content') {
    const form = document.getElementById('new-question-form');
    if (form) {
      form.reset();
      const choicesList = document.getElementById('new-choices-list');
      if (choicesList) {
        choicesList.innerHTML = '';
        const placeholder = document.createElement('div');
        placeholder.className = 'text-xs text-gray-400 italic';
        placeholder.id = 'no-choices-placeholder';
        placeholder.textContent = 'No hay opciones. Haz clic en "+ Agregar opción" para añadir.';
        choicesList.appendChild(placeholder);
      }
    }
  }

  // Reset new shop form
  if (event.detail.target && event.detail.target.id === 'shops-content') {
    const form = document.getElementById('new-shop-form');
    if (form) form.reset();
  }
});

// Question choice management - Event delegation for all question edit actions
document.addEventListener('change', function(e) {
  if (e.target.dataset.action === 'toggle-choices') {
    const questionId = e.target.dataset.questionId;
    const choicesSection = document.getElementById('choices-section-' + questionId);
...
document.addEventListener('click', function(e) {
  ...
  if (target.dataset.action === 'edit-shop') {
    e.preventDefault();
    const id = target.dataset.shopId;
    const name = target.dataset.shopName;
    const slug = target.dataset.shopSlug;
    const ciudadId = target.dataset.shopCiudad;
    editShop(id, name, slug, ciudadId);
  } else if (target.dataset.action === 'add-choice') {
    e.preventDefault();
    const questionId = target.dataset.questionId;
    const choicesList = document.getElementById('choices-list-' + questionId);

    // Remove placeholder if it exists
    const placeholder = document.getElementById('no-choices-placeholder');
    if (placeholder) placeholder.remove();

    // Count actual choice divs (not the placeholder)
    const actualChoices = choicesList.querySelectorAll('.flex.gap-2').length;

    const newChoice = document.createElement('div');
    newChoice.className = 'flex gap-2 items-center bg-gray-50 p-2 rounded';
    newChoice.innerHTML = `
      <input type="hidden" name="choice_ids[]" value="0">
      <input type="number" name="choice_orders[]" value="${actualChoices + 1}" placeholder="Orden" class="w-16 p-2 border border-gray-300 rounded text-xs focus:ring-2 focus:ring-pink-400">
      <input type="text" name="choice_values[]" placeholder="Valor (ej: 1)" class="w-24 p-2 border border-gray-300 rounded text-xs focus:ring-2 focus:ring-pink-400">
      <input type="text" name="choice_labels[]" placeholder="Etiqueta (ej: Malo)" class="flex-1 p-2 border border-gray-300 rounded text-xs focus:ring-2 focus:ring-pink-400">
      <button type="button" data-action="remove-choice" class="text-red-500 hover:text-red-700 px-2">✕</button>
    `;
    choicesList.appendChild(newChoice);

    // Focus on the first input of the new choice
    newChoice.querySelector('input[name="choice_values[]"]').focus();
  } else if (target.dataset.action === 'remove-choice') {
    e.preventDefault();
    target.closest('.flex.gap-2').remove();
  }
});
