...
(function() {
  ...
  function initDashboardCharts() {
    ...
    // Question Charts
    if (qStats) {
      Object.keys(qStats).forEach((question, index) => {
        if (index < 4) {
          // Find canvas by iterating question IDs (1, 2, 3, 5, etc.)
          const canvases = document.querySelectorAll('[id^="question-chart-"]');
          if (canvases[index]) {
            const qCtx = canvases[index];
            const qData = qStats[question];
